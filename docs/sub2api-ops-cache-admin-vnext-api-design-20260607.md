...
```json
{
  ...
  "data": {
    "task": {
      ...
      "provider": "responses",
      "model": "gpt-5.5",
      "started_at": "2026-06-07T12:00:05+08:00",
      "finished_at": "2026-06-07T12:00:30+08:00",
      "created_at": "2026-06-07T12:00:00+08:00",
      "updated_at": "2026-06-07T12:00:30+08:00"
    },
    "report": {
      "task_id": 123,
      "summary": "上游账号限流导致请求失败",
      "root_cause": "上游账号 429 限流",
      "impact_scope": {"affected_users": 2},
      "evidence": [{"text": "同类错误集中在 openai/gpt-5.5"}],
      "suggested_actions": ["切换可用上游账号", "检查账号限额"],
      "error_breakdown": {"upstream_rate_limit": 18},
      "confidence": "high",
      "feedback_status": "none",
      "created_at": "2026-06-07T12:00:30+08:00",
      "updated_at": "2026-06-07T12:00:30+08:00"
    }
  }
}
...
| task.status | report | 前端处理 |
|---|---|---|
| pending | 不返回 | 继续轮询，显示“待分析” |
| running | 不返回 | 继续轮询，显示“分析中” |
| completed | 有报告时返回 | 渲染报告卡片；若报告暂未写入，显示“报告生成中”并继续轮询 |
| failed | 不要求有报告 | 显示 `task.error_message` |
| expired | 可无报告 | 显示“报告已过期” |
...
| 404 | OPS_AI_ANALYSIS_TASK_NOT_FOUND | 任务不存在 |
| 403 | FORBIDDEN | 无权限查看 |
