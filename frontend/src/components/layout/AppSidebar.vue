...
function resolveCustomMenuPath(item: { id: string }): string {
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9A2.25 2.25 0 0015.75 16.5v-9A2.25 2.25 0 0013.5 5.25h-9A2.25 2.25 0 002.25 7.5v9A2.25 2.25 0 004.5 18.75z'
        })
      ]
    )
}
...
const flagModelSquare = makeSidebarFlag(FeatureFlags.modelSquare)
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/video-generation', label: t('nav.videoGeneration'), icon: VideoIcon },
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/model-square', label: t('nav.modelSquare'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagModelSquare },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/accounts', label: t('nav.accounts'), icon: GlobeIcon },
    {
      ...
      label: t('nav.upstreamMaintenance'),
      icon: ServerIcon,
      expandOnly: true,
      children: [
    ...
    { path: '/admin/finance', label: t('nav.financeStats'), icon: CreditCardIcon },
