<template>
  ...
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('auth.agreement.mustAgreeTitle') }}</p>
        <p class="mt-1 text-primary-700 dark:text-primary-200/80">
          {{ t('auth.agreement.mustAgreeDescription') }}
        </p>
      </div>
      <button
        type="button"
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
                  {{ t('auth.agreement.updatedDescription', { date: updatedAt || t('auth.agreement.recent') }) }}
                </p>
              </div>
            </div>
          </div>
...
const documentSeparator = computed(() => locale.value.startsWith('zh') ? '、' : ', ')
