      ...
      <div v-else-if="usageInfo" class="space-y-1">
        ...
        <div v-if="grokLocalUsage" class="mb-0.5 flex items-center">
          <div class="flex items-center gap-1.5 text-[9px] text-gray-500 dark:text-gray-400">
            ...
            <span v-if="grokLocalUsage.user_cost != null" class="rounded bg-gray-100 px-1.5 py-0.5 dark:bg-gray-800" :title="t('usage.userBilled')">
        ...
        <div v-if="grokQuotaUnknown" class="text-[10px] text-gray-500 dark:text-gray-400">{{ grokQuotaUnknownLabel }}</div>
        <div v-else-if="usageInfo.error" class="max-w-[200px] truncate text-xs text-amber-600 dark:text-amber-400" :title="usageInfo.error">{{ usageErrorLabel }}</div>
        <div v-if="grokQuotaStatusLine" class="text-[10px] text-gray-500 dark:text-gray-400">{{ grokQuotaStatusLine }}</div>
...
const grokIsFree = computed(() => {
  ...
  if (grokPlanLabelIsFree(plan) || grokPlanLabelIsFree(tier) || grokPlanLabelIsFree(entitlement)) return true
  return billing != null
})
...
const grokQuotaUnknownLabel = computed(() => usageInfo.value?.grok_quota_snapshot_state === 'no_headers'
...
const grokQuotaStatusLine = computed(() => {
  ...
  if (status) parts.push(t('admin.accounts.usageWindow.grokLastStatus', { status }))
  if (usageInfo.value?.grok_last_quota_probe_at) {
    parts.push(t('admin.accounts.usageWindow.grokLastProbe', { time: formatRelativeTime(usageInfo.value.grok_last_quota_probe_at) }))
  }
  if (usageInfo.value?.grok_last_headers_seen_at) {
    parts.push(t('admin.accounts.usageWindow.grokLastHeadersSeen', { time: formatRelativeTime(usageInfo.value.grok_last_headers_seen_at) }))
  }
...
const handleGrokProbed = (result: GrokQuotaProbeResult) => {
  ...
  if (!current) {
    loadUsage({ bypassCache: true }).catch((e) => console.error('Failed to refresh Grok usage:', e))
    return
  }
  ...
  const probeSucceeded = hasActiveProbeSnapshot && statusCode != null && statusCode >= 200 && statusCode < 300
  ...
  const merged: AccountUsageInfo = {
    ...
    grok_quota_snapshot_state: result.billing ? 'billing_observed' : snapshot?.headers_observed ? 'observed' : current.grok_quota_snapshot_state,
