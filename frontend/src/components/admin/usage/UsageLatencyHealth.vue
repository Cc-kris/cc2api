...
const firstTokenHealth = computed(() => latencyHealth(props.firstTokenMs, props.isError))
...
function healthTextClass(state: ReturnType<typeof latencyHealth>['state']) {
  return {
    ...
    'text-emerald-500 dark:text-emerald-400': state === 'good',
    'text-amber-500 dark:text-amber-400': state === 'slow',
    'text-red-500 dark:text-red-400': state === 'critical' || state === 'error',
  }
}
