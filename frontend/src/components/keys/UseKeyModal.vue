...
const platformNote = computed(() => {
  ...
  switch (props.platform) {
    ...
    case 'grok':
      if (activeClientTab.value === 'claude') return t('keys.useKeyModal.grok.claudeNote')
...
function generateGrokClaudeFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (activeTab.value) {
    case 'unix':
      path = 'Terminal'
      content = Object.entries(environment).map(([name, value]) => `export ${name}="${value}"`).join('\n')
      break
    case 'cmd':
      path = 'Command Prompt'
      content = Object.entries(environment).map(([name, value]) => `set ${name}=${value}`).join('\n')
      break
    case 'powershell':
      path = 'PowerShell'
      content = Object.entries(environment).map(([name, value]) => `$env:${name}="${value}"`).join('\n')
      break
    default:
      path = 'Terminal'
      content = ''
...
responses_websockets_v2 = true`
  ...
  return [
    ...
    { path: isWindows ? 'PowerShell' : 'Terminal', content: environmentContent }
  ]
}
