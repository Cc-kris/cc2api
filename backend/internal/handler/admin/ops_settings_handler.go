...
// ListAIAnalysisReportHistory returns generated AI analysis report history.
// GET /api/v1/admin/ops/ai-analysis/reports
func (h *OpsHandler) ListAIAnalysisReportHistory(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	if !canOperateOpsAIAnalysis(c) {
		response.Forbidden(c, "无权限查看 AI 分析报告")
		return
	}
	limit := 20
	if raw := strings.TrimSpace(c.Query("limit")); raw != "" {
	...
	items, err := h.opsService.ListAIAnalysisReportHistory(c.Request.Context(), limit)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
