	...
	for {
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				markOpsRoutingCapacityLimitedIfNoAvailable(c, err)
				h.errorResponse(c, http.StatusServiceUnavailable, "api_error", "No available compatible Grok accounts")
				return
			}
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if failoverErr.ShouldReportAccountScheduleFailure() {
					h.gatewayService.ReportOpenAIAccountScheduleModelResult(account.ID, grokMediaScheduleModel(account, routingModel, nil), false, nil)
				}
