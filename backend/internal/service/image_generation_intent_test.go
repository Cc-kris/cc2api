...
func TestPrepareCodexImageGenerationExtensionDispatch(t *testing.T) {
	...
	require.Contains(t, gjson.GetBytes(updated, "instructions").String(), "Copy every explicit output constraint verbatim")
	...
	require.Contains(t, gjson.GetBytes(updated, "tools.0.description").String(), "pixel dimensions, aspect ratio, output format")
	...
	require.True(t, IsSuccessfulCodexImageGenerationExtensionContinuation(continuation))
	continuationUpdated, changed, err := PrepareCodexImageGenerationExtensionDispatch(continuation)
	require.NoError(t, err)
	require.True(t, changed)
	require.False(t, gjson.GetBytes(continuationUpdated, "tools").Exists())
	...
	failedContinuation := []byte(`{"model":"gpt-5.6-terra","tools":[{"type":"namespace","name":"image_gen","tools":[{"type":"function","name":"imagegen"}]}],"input":[{"type":"function_call","namespace":"image_gen","name":"imagegen","call_id":"call_failed"},{"type":"function_call_output","call_id":"call_failed","output":"image generation failed: invalid response"}]}`)
	require.True(t, IsCodexImageGenerationExtensionContinuation(failedContinuation))
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(failedContinuation))
	failedUpdated, failedChanged, err := PrepareCodexImageGenerationExtensionDispatch(failedContinuation)
	require.NoError(t, err)
	require.True(t, failedChanged)
	require.False(t, gjson.GetBytes(failedUpdated, "tools").Exists())
	require.False(t, gjson.GetBytes(failedUpdated, "tool_choice").Exists())
	...
	require.True(t, IsSuccessfulCodexImageGenerationExtensionContinuation(outputOnlyContinuation))
	failedOutputOnlyContinuation := []byte(`{"model":"gpt-5.6-terra","previous_response_id":"resp_1","input":[{"type":"function_call_output","call_id":"call_1","output":"image generation failed: failed to decode image generation response"}]}`)
	require.True(t, IsCodexImageGenerationExtensionContinuation(failedOutputOnlyContinuation))
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(failedOutputOnlyContinuation))
	ordinaryToolOutputOnly := []byte(`{"model":"gpt-5.6-terra","previous_response_id":"resp_1","input":[{"type":"function_call_output","call_id":"call_exec","output":"command completed"}]}`)
	require.False(t, IsCodexImageGenerationExtensionContinuation(ordinaryToolOutputOnly))
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(ordinaryToolOutputOnly))
	ambiguousContinuation := []byte(`{"model":"gpt-5.6-terra","input":[{"type":"function_call","namespace":"image_gen","name":"imagegen","call_id":"call_ambiguous"},{"type":"function_call_output","call_id":"call_ambiguous","output":"saved"}]}`)
	require.True(t, IsCodexImageGenerationExtensionContinuation(ambiguousContinuation))
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(ambiguousContinuation))
	...
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(mismatched))
	...
	require.False(t, IsSuccessfulCodexImageGenerationExtensionContinuation(laterEdit))
