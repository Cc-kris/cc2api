...
// channel that owns the Codex text-to-image bridge. Codex WebSocket sessions
// multiplex prewarm, memory, and user turns on one connection, so handlers use
// this group-level decision before the WS handshake and force sticky HTTP
...
// validatePricingBillingMode 校验计费模式配置：按次/图片/按秒模式必须配价格或区间，所有价格字段不能为负，区间至少有一个价格字段。
	...
	if p.BillingMode == BillingModePerRequest || p.BillingMode == BillingModeImage || p.BillingMode == BillingModePerSecond {
		if p.PerRequestPrice == nil && len(p.Intervals) == 0 {
			return infraerrors.BadRequest(
				...
				"per-request price or intervals required for per_request/image/per_second billing mode",
			)
		}
	}
	return nil
