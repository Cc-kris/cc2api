...
func (s *OpenAIGatewayService) forwardGrokChatCompletionsViaResponses(
	...
	if clientStream {
		result, err = s.handleChatStreamingResponse(resp, c, account, originalModel, billingModel, upstreamModel, true, startTime)
	} else {
	...
	if result != nil {
		...
		result.ReasoningEffort = extractOpenAIReasoningEffortFromBody(body, upstreamModel)
	}
	return result, err
}
