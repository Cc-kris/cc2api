...
func TestPrepareUsageLogInsert_PersistsImmutableFinanceSnapshots(t *testing.T) {
	...
	legacySnapshot, ok := prepared.args[61].(string)
	require.True(t, ok)
	require.JSONEq(t, `{"version":"legacy","prices":{"input":"1.25000000"}}`, legacySnapshot)
	v2Snapshot, ok := prepared.args[62].(string)
	require.True(t, ok)
	require.JSONEq(t, `{"version":"v2"}`, v2Snapshot)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		...
		require.Equal(t, "task-image-metadata", *log.VideoTaskID)
	})
