...
func (r *dashboardAggregationRepository) CleanupUsageBillingDedup(ctx context.Context, cutoff time.Time) error {
	for {
		res, err := r.sql.ExecContext(ctx, `
			WITH victims AS (
				SELECT ctid, request_id, api_key_id, request_fingerprint, finance_business_type,
				       promotion_credit_used, finance_excluded, finance_exclusion_reason,
				       finance_classification_recorded, created_at
				FROM usage_billing_dedup
				WHERE created_at < $1
				LIMIT $2
			), archived AS (
				INSERT INTO usage_billing_dedup_archive (
					request_id, api_key_id, request_fingerprint, finance_business_type,
					promotion_credit_used, finance_excluded, finance_exclusion_reason,
					finance_classification_recorded, created_at
				)
				SELECT request_id, api_key_id, request_fingerprint, finance_business_type,
				       promotion_credit_used, finance_excluded, finance_exclusion_reason,
				       finance_classification_recorded, created_at
				FROM victims
