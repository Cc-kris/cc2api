...
func (r *usageLogRepository) GetSeedaceVideoByTaskID(ctx context.Context, apiKeyID int64, taskID string) (log *service.UsageLog, err error) {
	taskID = strings.TrimSpace(taskID)
	if taskID == "" {
		return nil, service.ErrUsageLogNotFound
	}
	query := "SELECT " + usageLogSelectColumns + " FROM usage_logs WHERE api_key_id = $1 AND video_task_id = $2 AND billing_mode = $3 ORDER BY id DESC LIMIT 1"
	rows, err := r.sql.QueryContext(ctx, query, apiKeyID, taskID, string(service.BillingModePerSecond))
	if err != nil {
		return nil, err
	}
	defer func() {
