...
func TestMigration145CreatesOpsAIAnalysisTasks(t *testing.T) {
	...
	require.Contains(t, sql, "CREATE INDEX IF NOT EXISTS idx_ops_ai_analysis_tasks_status_created")
	require.Contains(t, sql, "CREATE INDEX IF NOT EXISTS idx_ops_ai_analysis_tasks_source")
	require.Contains(t, sql, "CREATE INDEX IF NOT EXISTS idx_ops_ai_analysis_tasks_trigger_user")
	require.Contains(t, sql, "CREATE INDEX IF NOT EXISTS idx_ops_ai_analysis_tasks_time_range")
	require.Contains(t, sql, "pending/running/completed/failed/expired")
	require.NotContains(t, sql, "DROP TABLE")
	require.NotContains(t, sql, "DROP COLUMN")
